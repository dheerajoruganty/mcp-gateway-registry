...
def load_scopes_config():
    ...
    try:
        ...
        with open(scopes_file) as f:
            ...
            logger.info(
                f"Loaded scopes configuration from {scopes_file} with {len(config.get('group_mappings', {}))} group mappings"
            )
            return config
...
def validate_server_tool_access(
    ...
    try:
        ...
        logger.info(
            f"Available scopes config keys: {list(SCOPES_CONFIG.keys()) if SCOPES_CONFIG else 'None'}"
        )
    ...
    try:
        ...
        if original_url:
            try:
                ...
                logger.info(
                    f"Extracted server_name '{server_name_from_url}' from original_url: {original_url}"
                )
            except Exception as e:
                logger.warning(
        ...
        if server_name:
            ...
            method = (
                tool_name if tool_name else "initialize"
            )  # Default to initialize if no tool specified
...
async def generate_user_token(request: GenerateTokenRequest):
    ...
    try:
        ...
        logger.info(
            f"Generated access token for user '{hash_username(username)}' with scopes: {requested_scopes}, expires in {expires_in_hours} hours (no refresh token - configure longer token lifetime in Keycloak if needed)"
        )
...
async def oauth2_login(provider: str, request: Request, redirect_uri: str = None):
    ...
    try:
        ...
        if auth_server_external_url:
            ...
            logger.warning(
                f"AUTH_SERVER_EXTERNAL_URL not set, using dynamic URL: {auth_server_url}"
            )
...
async def oauth2_callback(
    ...
    try:
        ...
        if auth_server_external_url:
            ...
            auth_server_url = auth_server_external_url.rstrip("/")
            logger.info(
                f"Using configured AUTH_SERVER_EXTERNAL_URL for token exchange: {auth_server_url}"
            )
        else:
            # Fall back to dynamic construction (for development)
            host = request.headers.get("host", "localhost:8888")
        ...
        logger.info(
            f"Auth server setting session cookie: secure={cookie_secure} (config={cookie_secure_config}, is_https={is_https}), samesite={cookie_samesite}, domain={cookie_domain or 'not set'}, x-forwarded-proto={x_forwarded_proto}, request_scheme={request.url.scheme}"
        )
