...
################################################################################
# Initialize OpenSearch Serverless Indices
#
# This script:
# 1. Builds and pushes the opensearch-init Docker image to ECR
# 2. Reads terraform outputs from terraform-outputs.json
# 3. Creates an ECS task definition for opensearch-init container
# 4. Runs the task on the ECS cluster
# 5. Waits for task completion
# 6. Displays logs from CloudWatch
#
# Usage:
#   ./scripts/run-opensearch-init-task.sh [OPTIONS]
#
# Options:
#   --skip-build               Skip building and pushing Docker image
#   --aws-region REGION        AWS region (default: us-east-1)
#   --wait-timeout SECONDS     Timeout waiting for task (default: 300)
#   --recreate                 Delete and recreate indices if they exist
#   --help                     Show this help message
#
# Examples:
#   # Build image and run task (default)
#   ./scripts/run-opensearch-init-task.sh
#
#   # Skip build and run task only
#   ./scripts/run-opensearch-init-task.sh --skip-build
#
#   # Recreate indices if they exist
#   ./scripts/run-opensearch-init-task.sh --recreate
#
################################################################################
...
BUILD_SCRIPT="$SCRIPT_DIR/build-and-push-opensearch-init.sh"
...
log_info "=========================================="
log_info "OpenSearch Init ECS Task Runner"
log_info "=========================================="
log_info "AWS Region: $AWS_REGION"
log_info "Skip Build: $SKIP_BUILD"
log_info "Wait Timeout: $WAIT_TIMEOUT seconds"
log_info "Recreate Indices: ${RECREATE:-false}"
log_info "=========================================="

# Step 0: Build and push Docker image (optional)
if [[ "$SKIP_BUILD" == "false" ]]; then
    log_info "Step 0/7: Building and pushing opensearch-init Docker image..."
    ...
    if AWS_REGION="$AWS_REGION" bash "$BUILD_SCRIPT"; then
        ...
        IMAGE_URI="$(aws ecr describe-images \
            --repository-name mcp-gateway-opensearch-init \
            --region "$AWS_REGION" \
            --query 'sort_by(imageDetails, &imagePushedAt)[-1].imageTags[0]' \
            --output text 2>/dev/null)"
        ACCOUNT_ID="$(aws sts get-caller-identity --region "$AWS_REGION" --query Account --output text 2>/dev/null)"
        IMAGE_URI="${ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/mcp-gateway-opensearch-init:${IMAGE_URI}"
    ...
    IMAGE_TAG="$(aws ecr describe-images \
        ...
        --output text 2>/dev/null)"
    ACCOUNT_ID="$(aws sts get-caller-identity --region "$AWS_REGION" --query Account --output text 2>/dev/null)"
    IMAGE_URI="${ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/mcp-gateway-opensearch-init:${IMAGE_TAG}"
...
OPENSEARCH_ENDPOINT=$(jq -r '.opensearch_serverless_collection_endpoint.value // empty' "$OUTPUTS_FILE" 2>/dev/null)
if [[ -z "$OPENSEARCH_ENDPOINT" ]]; then
    log_error "Could not extract opensearch_serverless_collection_endpoint from terraform outputs"
    exit 1
fi
# Remove https:// prefix
OPENSEARCH_HOST=$(echo "$OPENSEARCH_ENDPOINT" | sed 's|https://||' | sed 's|http://||')
log_success "OpenSearch Host: $OPENSEARCH_HOST"
...
# Get execution role and task role from existing registry task
EXECUTION_ROLE=$(aws ecs describe-task-definition \
    --task-definition mcp-gateway-v2-registry \
    --region "$AWS_REGION" \
    --query 'taskDefinition.executionRoleArn' \
    --output text 2>/dev/null)

if [[ -z "$EXECUTION_ROLE" ]]; then
    log_error "Could not get execution role from registry task"
    exit 1
fi
...
if [[ -z "$TASK_ROLE" ]]; then
    log_error "Could not get task role from registry task"
    exit 1
fi
log_success "Task Role: $TASK_ROLE"

# Step 4: Create task definition
log_info "Step 4/6: Registering ECS task definition..."

# Build command array
CMD_ARRAY='["--host", "'"$OPENSEARCH_HOST"'", "--port", "443", "--use-ssl", "--auth-type", "aws_iam", "--region", "'"$AWS_REGION"'"'
if [[ -n "$RECREATE" ]]; then
    CMD_ARRAY+=', "--recreate"'
fi
CMD_ARRAY+=']'
...
TASK_DEF=$(cat <<EOF
{
  "family": "mcp-gateway-opensearch-init",
  "networkMode": "awsvpc",
  ...
  "containerDefinitions": [
    {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/mcp-gateway-opensearch-init",
...
TASK_DEF_FILE="/tmp/mcp-gateway-opensearch-init-taskdef-$$.json"
...
if ! aws logs describe-log-groups --log-group-name-prefix /ecs/mcp-gateway-opensearch-init --region $AWS_REGION --query 'logGroups[0].logGroupName' 2>/dev/null | grep -q "mcp-gateway-opensearch-init"; then
    log_info "Creating CloudWatch log group..."
    aws logs create-log-group \
        --log-group-name "/ecs/mcp-gateway-opensearch-init" \
...
TASK_ARN=$(aws ecs run-task \
    --cluster "$CLUSTER_NAME" \
    --task-definition "mcp-gateway-opensearch-init" \
    --launch-type FARGATE \
...
LOG_STREAM="ecs/opensearch-init/$TASK_ID"
...
echo ""
log_success "=========================================="
log_success "OpenSearch Init Task Complete!"
log_success "=========================================="
log_info "Task ARN: $TASK_ARN"
log_info "Exit Code: $EXIT_CODE"
log_info ""
log_info "The OpenSearch Serverless indices should now be initialized."
log_info ""
